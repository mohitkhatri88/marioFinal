...
public class DataRecorder {
	...
	private Level level;
	private RandomLevel levelR;
	private MyLevel levelM;
	...
	public void endRightMoveRecord(){
		...
		System.out.println("RightMove: StTime = "+startRightTime +" EdTime = "+endRightTime + " TtTime = " + totalRightTime);
		detailedLog += "RightMove: StTime = "+startRightTime +" EdTime = "+endRightTime + " TtTime = " + totalRightTime;
		detailedLog += "\n";
	}
	...
	public void endLeftMoveRecord(){
		...
		System.out.println("LeftMove: StTime = "+startLeftTime +" EdTime = "+endLeftTime + " TtTime = " + totalLeftTime);
		detailedLog += "LeftMove: StTime = "+startLeftTime +" EdTime = "+endLeftTime + " TtTime = " + totalLeftTime;
		detailedLog += "\n";
...
//			levelScene.levelRecorder.enemyR.interact.add(new DataEntry(EnemyRecorder.GREEN_TURTLE, EnemyRecorder.UNLEASHED, 2982 - levelScene.timeLeft,x,y));
	...
	public double nb(){
		double n = 0;
		if(levelR != null){
			/*if( levelR.BLOCKS_EMPTY != 0)
				n+= getBlocksEmptyDestroyed()/levelR.BLOCKS_EMPTY;
			if(levelR.BLOCKS_POWER != 0)
				n+= getBlocksPowerDestroyed()/levelR.BLOCKS_POWER;
			if( levelR.BLOCKS_COINS != 0)
				n+= getBlocksCoinDestroyed()/levelR.BLOCKS_COINS;*/
			if(levelR.BLOCKS_COINS + levelR.BLOCKS_EMPTY + levelR.BLOCKS_POWER != 0){
				n = (double)((double)(getBlocksEmptyDestroyed() + getBlocksPowerDestroyed() + getBlocksCoinDestroyed()) / (double)(levelR.BLOCKS_EMPTY + levelR.BLOCKS_POWER + levelR.BLOCKS_COINS));
			}
		}else if(levelM != null){
			/*if( levelM.BLOCKS_EMPTY != 0)
				n+= getBlocksEmptyDestroyed()/levelM.BLOCKS_EMPTY;
			if(levelM.BLOCKS_POWER != 0)
				n+= getBlocksPowerDestroyed()/levelM.BLOCKS_POWER;
			if( levelM.BLOCKS_COINS != 0)
				n+= getBlocksCoinDestroyed()/levelM.BLOCKS_COINS;*/
			if(levelM.BLOCKS_COINS + levelM.BLOCKS_EMPTY + levelM.BLOCKS_POWER != 0){
				n = (double)((double)(getBlocksEmptyDestroyed() + getBlocksPowerDestroyed() + getBlocksCoinDestroyed()) / (double)(levelM.BLOCKS_EMPTY + levelM.BLOCKS_POWER + levelM.BLOCKS_COINS));
			}
		}
	...
	public double ncb(){
		if(levelR != null){
			if( levelR.BLOCKS_COINS != 0)
				return (double)getBlocksCoinDestroyed()/(double)levelR.BLOCKS_COINS;
			else
				return 0;
		}else if(levelM != null){
			if( levelM.BLOCKS_COINS != 0)
				return (double)getBlocksCoinDestroyed()/(double)levelM.BLOCKS_COINS;
			else
				return 0;
		}else{
			return 0;
	...
	public double neb(){
		if(levelR != null){
			if( levelR.BLOCKS_EMPTY != 0)
				return (double)getBlocksEmptyDestroyed()/(double)levelR.BLOCKS_EMPTY;
			else
				return 0;
		}else if(levelM != null){
			if( levelM.BLOCKS_EMPTY != 0)
				return (double)getBlocksEmptyDestroyed()/(double)levelM.BLOCKS_EMPTY;
			else
				return 0;
		}else{
			return 0;
	...
	public double np(){
		if(levelR != null){
			if( levelR.BLOCKS_POWER != 0)
				return (double)getBlocksPowerDestroyed()/(double)levelR.BLOCKS_POWER;
			else
				return 0;
		}else if(levelM != null){
			if( levelM.BLOCKS_POWER != 0)
				return (double)getBlocksPowerDestroyed()/(double)levelM.BLOCKS_POWER;
			else
				return 0;
		}else{
			return 0;
