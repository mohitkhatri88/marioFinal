...
import dk.itu.mario.level.MyLevel.ComponentType;
import dk.itu.mario.level.MyLevel.Profile;
import dk.itu.mario.level.MyLevel.SANode;


public class MyLevelSA extends Level{
	//Store information about the level
	 ...
	 /*
	  * Node which makes up the values for the linkedlist used in simulated annealing. 
	  */
	 public class SANode {
		 public ComponentType componentType;
		 public int length;
		 
		 public SANode(ComponentType componentType) {
			 this.componentType = componentType;
		 }
	 }
	 
	 public void creatSA(long seed, int difficulty, int type)
	    {
	        ...
	        LinkedList<SANode> saMap = new LinkedList<SANode>();

	        //create the start location
	        int length = 0;
	        length += buildStraight(0, width, true);
	        
	        //Add the basic structure of the map to the linked list. 
	        //Add a start Location to the LinkedList
	        saMap.add(new SANode(ComponentType.STRAIGHT));
	        
	        //create all of the medium sections based on the profile that we have earlier generated. 
	        while (length < width - 64)
	        {
	            //length += buildZoneNeutral(length, width - length);
		        double t = random.nextInt(7);
		    	
				if(t==0){
					length += buildHillStraight(length, width - length);
					saMap.add(new SANode(ComponentType.HILL));
					this.playerProfile.hillCount++;
				}
				else if(t==1){
					length += buildJump(length, width - length);
					saMap.add(new SANode(ComponentType.GAP));
					this.playerProfile.jumpCount++;
				}
				else if(t==2){
					length += buildTubes(length, width - length);
					saMap.add(new SANode(ComponentType.TUBE));
					this.playerProfile.tubeCount++;
				}
				else if(t==3){
					length +=buildCannons(length, width - length);
					saMap.add(new SANode(ComponentType.CANNON));
					this.playerProfile.cannonCount++;
				}
				else if(t==4){
					length += buildStraight(length, width - length, false);
					saMap.add(new SANode(ComponentType.STRAIGHT));
					this.playerProfile.straightCount++;
				} else if(t==5){
					length += buildStraight(length, width - length, false);
					saMap.add(new SANode(ComponentType.STRAIGHT));
					this.playerProfile.enemiesCount++;
				} else if(t==6){
					length += buildStraight(length, width - length, false);
					saMap.add(new SANode(ComponentType.STRAIGHT));
					this.playerProfile.coinCount++;
				}
	        }
	        
	        //Send the linked list to the simulated Annealing algorithm to get the right structure for the current profile.  
	        saMap = SimulatedAnnealing(saMap);
	        
	        /* Now to figure out the actual configuration, we start with a straight node, and then use the values that we 
	         * get from the linked list till that runs out or we reach the ending condition. At that, we use the normal creation code to 
	        ...
	        while (length < width - 64) {
	        	if (!saMap.isEmpty()) {
	        		//We remove the values from the map as and when we create a module, and then use it to 
	        		length += buildZoneSA(length, width - length, saMap.get(0));
	        		saMap.remove(0);
	        	} else {
	 ...
	 /*
		 * Simulated annealing on the given LinkedList and provide the right LInkedList based on the evaluation functions. 
		 */
	    public LinkedList<SANode> SimulatedAnnealing(LinkedList<SANode> saMap) {
	    	int nIter = 1000;
	    	int startTemp = 500;
	    	int k = 0;
	    	
	    	
	    	//Get an acceptable score based on the current profile. 
	    	int acceptableScore = getAcceptableScore(saMap);
	    	
	    	int currentScore = evalScore(saMap);
	    	//Acceptable score is one of the parameters that must be satisfied. Along with that, 
	    	while (currentScore < acceptableScore && k < nIter) {
	    		double temp = getTemperature(startTemp, (k/nIter));
	    		LinkedList<SANode> currentMap = saMap;
	    		int index = random.nextInt(currentMap.size());
	    		SANode currentNode = currentMap.get(index);
	    		currentMap.remove(index);
	    		currentNode.componentType = ComponentType.values()[random.nextInt(7)];
	    		if (index == currentMap.size() ) {
	    			index--;
	    		}
	    		currentMap.add(index, currentNode);
	    		int newScore = evalScore(currentMap);
	    ...
	    public double getTemperature(int startTemp, double iter) {
	    	return startTemp * (1-iter);
	    }
	    
	    public int evalScore(LinkedList<SANode> saMap) {
	    	int result = 0;
	    	for (int k=0;k<saMap.size();k++) {
	    		SANode currentNode = saMap.get(k);
	    		if (this.playerProfile.profile == Profile.COINKEEPER) {
		    		//Coin keeper should be able to maintain a percentage of 70%
	    			if (currentNode.componentType == ComponentType.COIN) {
	    				result +=3;
	    			} else {
	    				result +=1;
	    			}
		    		
		    	} else if (this.playerProfile.profile == Profile.JUMPER) {
		    		if (currentNode.componentType == ComponentType.GAP) {
	    				result +=5;
	    			} else {
	    				result +=1;
	    			}
		    	} else if (this.playerProfile.profile == Profile.SPEEDY) {
		    		//Speedy has at minimum 80 percentage of stuff that isnt cannon and tubes.
		    		if (currentNode.componentType == ComponentType.TUBE || currentNode.componentType == ComponentType.CANNON) {
	    				result +=1;
	    			} else {
	    				result +=5;
	    			}
		    	} else if (this.playerProfile.profile == Profile.KILLER) {
		    		//Killer should be able to maintain a percentage of 80%
		    		if (currentNode.componentType == ComponentType.COIN) {
	    				result +=3;
	    			} else {
	    				result +=1;
	    ...
	    public int getAcceptableScore(LinkedList<SANode> saMap) {
	    	 int result = 0;
	    	if (this.playerProfile.profile == Profile.COINKEEPER) {
	    		//Coin keeper should be able to maintain a percentage of 70%
	    		result = (int)0.7*saMap.size()*3;
	    	} else if (this.playerProfile.profile == Profile.JUMPER) {
	    		//JUmper has 30 percentage of gaps. 
	    		result = (int)0.3*saMap.size()*5;
	    	} else if (this.playerProfile.profile == Profile.SPEEDY) {
	    		//Speedy has at minimum 80 percentage of stuff that isnt cannon and tubes. 
	    		result = (int)0.8*saMap.size()*5;
	    	} else if (this.playerProfile.profile == Profile.KILLER) {
	    		//Killer should be able to maintain a percentage of 80%
	    		result = (int)0.8*saMap.size()*3;
	    	}
	    	return result;
	    }
	    
		public int buildZoneSA(int x, int maxLength, SANode saNode) {
			ComponentType type = saNode.componentType;
			decideParameters(saNode);
			if(type == ComponentType.CANNON){
				return buildCannons(x, maxLength);
			}
			else if(type == ComponentType.COIN){
				return buildZone(x, maxLength); 
			}
			else if(type == ComponentType.KILL){
				return buildZone(x, maxLength);
			}
			else if(type == ComponentType.GAP){
				return buildJump(x, maxLength);
			}
			else if(type == ComponentType.HILL){
		...
		public void decideParameters(SANode saNode) {
			 this.playerProfile.hillProbability = 2.0;
			 this.playerProfile.jumpProbability = 4.0;
			 this.playerProfile.tubesProbability = 6.0;
			 this.playerProfile.cannonsProbability = 8.0;
			 this.playerProfile.straightProbability =  10.0;
			 if (saNode.componentType == ComponentType.KILL) {
				 this.playerProfile.enemiesProbability = 0.8;
			 } else if (saNode.componentType == ComponentType.COIN) {
				 this.playerProfile.coinProbability = 0.8;
			 }
		}
	 ...
	 public class PlayerProfile {
		 ...
		 //Variables to keep track of the number of modules that 
